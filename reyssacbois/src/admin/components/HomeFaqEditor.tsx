...
export default function HomeFaqEditor({
  ...
  return (
    <section className="rounded-3xl border border-line bg-surface p-4 sm:p-6 shadow-[0_20px_80px_-60px_rgba(0,0,0,0.55)] ring-1 ring-black/10">
      <h3 className="text-sm font-semibold text-ink">Home — FAQ</h3>
      ...
      {portalReady && isAddOpen
        ? createPortal(
            <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
              ...
              />
              <div
                className="relative z-10 w-full max-w-2xl rounded-3xl border border-line bg-surface p-4 sm:p-6 shadow-[0_40px_140px_-90px_rgba(0,0,0,0.75)] ring-1 ring-black/10"
                role="dialog"
                aria-modal="true"
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <button
                    type="button"
                    className="inline-flex items-center justify-center rounded-full border border-line bg-white px-3 py-1.5 text-xs font-semibold text-ink hover:bg-surface-2"
                    onClick={() => setIsAddOpen(false)}
                  >
                    Fermer
                  </button>
                ...
                <div className="mt-5 space-y-4">
                  <label className="inline-flex items-center gap-2 rounded-full border border-line bg-surface px-3 py-1.5 text-xs font-medium text-ink">
                    <input
                      type="checkbox"
                ...
                <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-end">
                  ...
                  <button
                    type="button"
                    disabled={!canAdd}
                    className="inline-flex items-center justify-center rounded-lg bg-forest-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-forest-800 focus:outline-none focus:ring-2 focus:ring-forest-700/30 disabled:opacity-60"
                    onClick={() => {
                      if (!canAdd) return;
                      const id = newId();
      ...
      <div className="mt-4 space-y-4">
        {items.map((it, idx) => (
          <div
            key={it.id}
            className="rounded-3xl border border-line/70 bg-surface p-4 sm:p-5 shadow-sm ring-1 ring-black/5"
          >
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <p className="text-xs font-semibold tracking-wide text-forest-800/90">
                  Question {idx + 1}
                </p>
              </div>
