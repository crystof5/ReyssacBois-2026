...
export default function HomeFaqEditor({
  ...
  const [faqVisible, setFaqVisible] = useState<boolean>(Boolean(initial.isVisible))
  ...
  const idsJson = useMemo(() => JSON.stringify(items.map((i) => i.id)), [items])
  ...
  return (
    <section className="rounded-3xl border border-white/20 bg-white/70 p-4 sm:p-6 shadow-[0_20px_80px_-60px_rgba(0,0,0,0.55)] ring-1 ring-black/10 backdrop-blur-xl">
      ...
      <p className="mt-1 text-xs text-gray-500">
        Cette section s’affiche sur la page d’accueil juste avant le contact. Les réponses sont en RichText (liens, gras,
        listes…).
      </p>

      <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <label className="block">
          <span className="mb-1 block text-sm font-medium text-gray-900">Afficher la section FAQ</span>
          <select
            name="faqVisible"
      ...
      <div className="mt-4">
        <label className="block">
          <span className="mb-1 block text-sm font-medium text-gray-900">Texte d’introduction</span>
          <p className="mt-1 text-xs text-gray-500">Affiché sous le titre. Mise en forme possible.</p>
        </label>
        <div className="mt-3">
          <RichTextEditor
      ...
      <div className="mt-4 space-y-4">
        {items.map((it, idx) => (
          ...
          >
            ...
            <div className="mt-4">
              <label className="block">
                <span className="mb-1 block text-sm font-medium text-gray-900">Question</span>
                <input
                  name={`faqQuestion_${it.id}`}
                  value={it.question}
                  onChange={(e) => {
                    const v = e.currentTarget.value
                    setItems((prev) => prev.map((x) => (x.id === it.id ? { ...x, question: v } : x)))
                  }}
                  className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-green-600/20"
            ...
            <div className="mt-4">
              <label className="block">
                <span className="mb-1 block text-sm font-medium text-gray-900">Réponse (RichText)</span>
              </label>
              <div className="mt-2">
                <RichTextEditor
