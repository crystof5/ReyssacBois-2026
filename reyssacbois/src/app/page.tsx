...
export default async function Home() {
  ...
  const aboutHistoryImage = aboutHistory ?? { src: "/img/placeholder.svg", alt: "Histoire Reyssac Bois" }
  ...
  const aboutPageTitle = aboutTexts?.pageTitle?.trim() ? aboutTexts.pageTitle.trim() : "Qui sommes-nous ?"
  const aboutHistoryTitle = aboutTexts?.historyTitle?.trim() ? aboutTexts.historyTitle.trim() : "Notre Histoire"
  ...
  const aboutMissionTitle = aboutTexts?.missionTitle?.trim() ? aboutTexts.missionTitle.trim() : "Notre Mission"
  ...
  return (
    <div className="min-h-screen">
      {/* ACCUEIL / HERO */}
      <section id="accueil" className="relative overflow-hidden scroll-mt-24 min-h-[58vh]">
        ...
        <Container className="relative z-10 py-16 sm:py-24 lg:py-28">
          <div className="max-w-3xl rounded-3xl border border-white/10 bg-black/25 p-6 sm:p-8 backdrop-blur shadow-[0_18px_45px_-30px_rgba(0,0,0,0.9)]">
            ...
            <div className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-3">
              <HeroPill title="Depuis 1850" desc="Entreprise familiale" />
              <HeroPill title="Conseil" desc="Pro & particuliers" />
              <HeroPill title="Stock" desc="Disponibilité rapide" />
            </div>
          </div>
        </Container>
      </section>
        ...
        <Container>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            ...
            <div className="space-y-6 rounded-3xl border border-white/10 bg-white/35 p-6 sm:p-7 backdrop-blur shadow-sm ring-1 ring-black/5">
              <div>
                <p className="text-xs font-semibold tracking-wide text-green-800/90">REYSSAC BOIS</p>
                <h2 className="mt-2 text-3xl sm:text-4xl font-bold text-gray-900">{familyTitle}</h2>
              </div>
              ...
              <div className="space-y-4 text-gray-700">
                {familyP1Html ? (
                  ...
                  <p className="text-base sm:text-lg leading-relaxed">{familyP1Fallback}</p>
                )}
                {familyP2Html ? (
                  <RichText html={familyP2Html} className="text-base sm:text-lg" />
                ) : (
                  <p className="text-base sm:text-lg leading-relaxed">{familyP2Fallback}</p>
                )}
              </div>
      ...
      <section id="catalogue" className="py-14 sm:py-20 scroll-mt-24">
        <Container>
          <div className="flex flex-col items-center text-center">
            ...
            <h2 className="mt-2 text-3xl sm:text-4xl font-bold text-gray-900">Nos produits</h2>
          ...
          <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
            <TeaserCard title="Bois de construction" desc="Sections, longueurs, essences: des bases solides pour vos chantiers." />
            <TeaserCard title="Aménagement extérieur" desc="Terrasse, bardage, clôture: du durable, du beau, du conseillé." />
            <TeaserCard title="Quincaillerie & traitement" desc="Fixations, colles, saturateurs… le bon produit au bon usage." />
          </div>
      ...
      {/* QUI SOMMES-NOUS */}
      <section id="qui-sommes-nous" className="py-14 sm:py-20 border-y border-white/10 bg-white/30 backdrop-blur scroll-mt-24">
        <Container>
          <div className="flex flex-col items-center text-center">
            <p className="text-xs font-semibold tracking-wide text-green-800/90">À PROPOS</p>
            <h2 className="mt-2 text-3xl sm:text-4xl font-bold text-gray-900">{aboutPageTitle}</h2>
            <p className="mt-3 max-w-2xl text-sm sm:text-base text-gray-700">
              Une entreprise familiale, une expertise transmise, et une exigence de qualité au quotidien.
            </p>
          </div>
          ...
          <div className="mt-10 grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
            <div className="rounded-3xl border border-white/20 bg-white/55 backdrop-blur shadow-sm ring-1 ring-black/5 p-6 sm:p-7">
              <h3 className="text-xl sm:text-2xl font-bold text-gray-900">{aboutHistoryTitle}</h3>
              {aboutHistoryTextHtml ? (
                <div className="mt-4">
                  <RichText html={aboutHistoryTextHtml} className="text-gray-700 text-base sm:text-lg" />
                </div>
              ) : (
                <p className="mt-4 text-gray-700 text-base sm:text-lg leading-relaxed">{aboutHistoryText}</p>
              )}

              <div className="mt-6 overflow-hidden rounded-2xl border border-gray-200/70 bg-white/50">
                <div className="aspect-[4/3] w-full">
                  <Media src={aboutHistoryImage.src} alt={aboutHistoryImage.alt} className="h-full w-full" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="rounded-3xl border border-white/20 bg-white/55 backdrop-blur shadow-sm ring-1 ring-black/5 p-6 sm:p-7">
                <h3 className="text-xl sm:text-2xl font-bold text-gray-900">{aboutMissionTitle}</h3>
                {aboutMissionTextHtml ? (
                  <div className="mt-4">
                    <RichText html={aboutMissionTextHtml} className="text-gray-700 text-base sm:text-lg" />
                  </div>
                ) : (
                  <p className="mt-4 text-gray-700 text-base sm:text-lg leading-relaxed">{aboutMissionText}</p>
                )}
              </div>

              <div className="rounded-3xl border border-white/20 bg-white/55 backdrop-blur shadow-sm ring-1 ring-black/5 p-6 sm:p-7">
                <h3 className="text-xl sm:text-2xl font-bold text-gray-900">{aboutLocationTitle}</h3>
                {aboutLocationTextHtml ? (
                  <div className="mt-4">
                    <RichText html={aboutLocationTextHtml} className="text-gray-700 text-base sm:text-lg" />
                  </div>
                ) : (
                  <p className="mt-4 text-gray-700 text-base sm:text-lg leading-relaxed">{aboutLocationText}</p>
                )}
              </div>

              <div className="rounded-3xl border border-white/20 bg-white/55 backdrop-blur shadow-sm ring-1 ring-black/5 p-6 sm:p-7">
                {aboutConclusionTextHtml ? (
                  <RichText html={aboutConclusionTextHtml} className="text-gray-800 text-base sm:text-lg text-center" />
                ) : (
                  <p className="text-gray-800 text-base sm:text-lg text-center">{aboutConclusionText}</p>
                )}
              </div>
            </div>
          </div>
      ...
      <section id="contact" className="py-14 sm:py-20 scroll-mt-24">
        <Container>
          <div className="flex flex-col items-center text-center">
            <p className="text-xs font-semibold tracking-wide text-green-800/90">CONTACT</p>
            <h2 className="mt-2 text-3xl sm:text-4xl font-bold text-gray-900">Parlons de votre projet</h2>
            <p className="mt-3 max-w-2xl text-sm sm:text-base text-gray-700">
              Disponibilité, devis, conseil, commande: décrivez votre besoin et nous vous répondons rapidement.
            </p>
          </div>
          ...
          <div className="mt-10 grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            <div className="space-y-6">
              <div className="rounded-3xl border border-gray-200/70 bg-white/55 backdrop-blur p-6 shadow-sm ring-1 ring-black/5">
                <h3 className="text-base font-semibold text-gray-900">Informations</h3>
...
function TeaserCard({ title, desc }: { title: string; desc: string }) {
  return (
    <div className="group rounded-3xl border border-gray-200/70 bg-white/55 p-6 shadow-sm ring-1 ring-black/5 backdrop-blur transition hover:-translate-y-0.5 hover:shadow-[0_18px_45px_-28px_rgba(0,0,0,0.55)] hover:ring-black/10 hover:bg-white/60">
