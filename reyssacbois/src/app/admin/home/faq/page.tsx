...
import { ensureHomeFaqSettings, getSettingsVersion, SITE_KEYS } from "@/admin/queries/siteSettings";
import FaqForm from "./FaqForm";

export default async function AdminHomeFaqPage() {
  const faq = await ensureHomeFaqSettings();
  const version = await getSettingsVersion([SITE_KEYS.homeFaq]);
    ...
    <div>
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-ink-600">
              Questions fréquentes sur la Home.
            </p>
      ...
      <FaqForm
        ...
        initial={{
          ...
          title: faq?.title ?? "",
          introHtml: faq?.introHtml ?? faq?.intro ?? "",
          items: (faq?.items ?? []).map((it) => ({
            id: it.id,
