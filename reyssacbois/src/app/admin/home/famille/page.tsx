...
export default async function AdminHomeFamillePage() {
  const [family, homeTexts] = await Promise.all([
    getSiteImage(SITE_KEYS.homeFamily),
    ensureHomeTexts(),
  ]);
  const version = await getSettingsVersion([SITE_KEYS.homeFamily, SITE_KEYS.homeTexts]);
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">
              Accueil — Une histoire de famille
            </h2>
            <p className="mt-1 text-sm text-gray-700">Image + titre + texte.</p>
