...
export default async function AdminHomeAboutPage() {
  const [aboutHistory, aboutTexts] = await Promise.all([
    getSiteImage(SITE_KEYS.aboutHistory),
    ensureAboutTexts(),
  ]);
  const version = await getSettingsVersion([SITE_KEYS.aboutHistory, SITE_KEYS.aboutTexts]);
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-ink-600">
              Section “Qui sommes-nous” (Notre histoire, mission…).
            </p>
            ...
            <nav aria-label="Fil d’Ariane admin" className="mt-3">
              <ol className="inline-flex max-w-full flex-wrap items-center gap-2 border-b border-line pb-2 text-xs text-ink-600">
                ...
                <li className="flex min-w-0 items-center gap-1">
                  ...
                  <span className="font-semibold text-ink">À propos</span>
                </li>
              </ol>
            </nav>
          </div>
